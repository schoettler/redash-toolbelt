...
def import_users():
    ...
    for user in users:
        ...
        response = requests.post(
            DESTINATION + "/api/users?no_invite=1",
            json=data,
...
def get_api_key(user_id):
    response = requests.get(
        DESTINATION + "/api/users/{}".format(user_id),
        headers=auth_headers(DESTINATION_API_KEY),
    )
    response.raise_for_status()
...
def import_queries():
    ...
    for query in queries:
        ...
        if not query["is_draft"]:
            response = requests.post(
                DESTINATION + "/api/queries/" + str(new_query_id),
                json={"is_draft": False},
                headers=auth_headers(user["api_key"]),
            )
            response.raise_for_status()
...
def import_visualizations():
    ...
    for query_id, new_query_id in meta["queries"].items():
        ...
        for v in query["visualizations"]:
            if v["type"] == "TABLE":
                response = requests.get(
                    DESTINATION + "/api/queries/{}".format(new_query_id),
                    headers=auth_headers(DESTINATION_API_KEY),
                )
                response.raise_for_status()
                ...
                response = requests.post(
                    DESTINATION + "/api/visualizations",
                    json=data,
                    headers=auth_headers(user["api_key"]),
                )
                response.raise_for_status()
...
def import_dashboards():
    ...
    for dashboard in dashboards:
        ...
        requests.post(
            DESTINATION + "/api/dashboards/{}".format(new_dashboard["id"]),
            json={"is_draft": False},
            headers=auth_headers(user["api_key"]),
        )
